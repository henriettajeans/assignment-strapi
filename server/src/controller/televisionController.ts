...
import  * as data from "../../strapisource/television.api"
   ...
   export const  getTelevisionById = async(req: Request, res: Response)  =>{
        ...
        return res.status(500).json({message: ` failed to find a television with id ${id} ⌛️`})
        }
    }
    ...
    export const createTelevision =  async(req:Request, res:Response)  =>{
        ...
        if(record.data.name){
            res.status(409).json({message: `A television with name ${record.data.name} already exists ❤️‍🩹`})
        }else{
            res.status(201).json({message: `added a new television 🪴`})
        }

        }catch(e){
        res.status(500).json({message: `failed to add a new television 🤔`})
    ...
    export const  deletedTelevisionById = async(req: Request, res: Response)  =>{
        ...
        return res.status(200).json({ record: deletedRecord, message: `deleted television with id ${id} 👍`})
        }
        
        }catch(e){
        return res.status(500).json({message: `failed to delete a television with id ${id} 🤨`})
        }
    ...
    export const updateTelevision =  async(req:Request, res:Response)  =>{
        ...
        try{
            ...
            if(!foundRecord){
                return res.status(404).json({message: `Television with ${id} not found 🤯`})
            }else{
        ...
        res.status(500).json({message: `failed to update television with ${id} 🥺`})
        }
    }