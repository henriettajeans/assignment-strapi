...
import  * as data from "../../strapisource/mobile.api"
   ...
   export const  getMobileById = async(req: Request, res: Response)  =>{
    ...
    export const createMobile =  async(req:Request, res:Response)  =>{
        ...
        if(record.data.name){
            res.status(409).json({message: ` mobile with name ${record.data.name} already exists ❤️‍🩹`})
        }else{
            res.status(201).json({message: `added a new mobile 🪴`})
        }

        }catch(e){
        res.status(500).json({message: ` failed to add a new mobile 🤔`})
    ...
    export const  deletedMobileById = async(req: Request, res: Response)  =>{
        ...
        return res.status(200).json({ record: deletedRecord, message: `deleted mobile with id ${id} 👍`})
        }
        
        }catch(e){
        return res.status(500).json({message: ` failed to delete mobile with id ${id} 🤨`})
        }
    ...
    export const updateMobile =  async(req:Request, res:Response)  =>{
        ...
        res.status(500).json({message: `failed to update mobile with ${id} 🥺`})
